...
import {
  ...
  deleteCategoryByIdUsingPost,
  queryCategoryPageListUsingPost,
  updateCategoryUsingPost,
} from '@/api/categoryController.ts'
import CategoryFormModal from '@/pages/admin/category/CategoryFormModal.vue'
...
// 定义数据
const dataList = ref<API.CategoryPageListVO[]>([])
// 总条数
const total = ref(0)
//分页
...
const handleSearch = async () => {
  const res = await queryCategoryPageListUsingPost({ ...searchParams })
...
const onChangeSort = async (
  pagination: unknown,
  filters: unknown,
  sorter: SorterResult<API.CategoryPageListVO> | SorterResult<API.CategoryPageListVO>[]
)  => {
  if (Array.isArray(sorter)) {
    ...
    const singleSorter = sorter as SorterResult<API.CategoryPageListVO>
...
const handleDelete = async (id: number) => {
  const res = await deleteCategoryByIdUsingPost({ id })
  if (res.data.code === 0) {
...
// 编辑
const editingCategory = ref<API.EditCategoryDTO | null>(null)
const modalVisible = ref(false)

const handleEdit = (dto: API.EditCategoryDTO) => {
  editingCategory.value = { ...dto }
...
// 表单提交
const handleSubmit = async (dto: API.EditCategoryDTO) => {
  if (dto.id) {
    // 编辑
    const res = await updateCategoryUsingPost(dto)
    if (res.data.code === 0) {
      clearDto(dto)
    ...
    // 新增
    const res = await addCategoryUsingPost(dto)
    if (res.data.code === 0) {
      clearDto(dto)
...
const clearDto = (dto: API.EditCategoryDTO) => {
  dto.id = undefined
  dto.name = ''
  dto.sort = undefined
}
